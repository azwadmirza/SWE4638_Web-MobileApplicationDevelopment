...
import { getQuizById, patchQuiz, postQuiz } from "../../../../lib/requests";
...
export const useEditQuestion=(id)=>{
        ...
        if(id){
            getQuizById(id).then((data) => {
                data.json().then((data) => {
                    ...
                    setHours(data.quiz.time.hours);
                    setMinutes(data.quiz.time.minutes);
                    setSeconds(data.quiz.time.seconds);
    ...
    const saveQuiz = async() => {
        ...
        await patchQuiz(id,{title,author:localStorage.getItem("username"),time:{hours,minutes,seconds},questions})
        router.push('/auth');
    }
